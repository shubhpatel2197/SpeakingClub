...
export default function ChatPanel({
  ...
  return (
    <div
      className={`flex flex-col bg-background text-foreground ${isMdUp
    ...
    >
      ...
      <div className="flex justify-between items-center px-4 py-3 border-b border-border bg-card">
        ...
        <button
          onClick={onClose}
          className="p-1 rounded-full hover:bg-white/5 transition-colors text-foreground/60 hover:text-foreground"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
          ...
          <div className="space-y-2">
            {displayMessages.map((m) => (
              ...
              >
                ...
                <div
                  className={`max-w-[78%] px-3 py-2 rounded-xl ${m.isSelf
      ...
      <div
        ref={composerRef}
        className={`p-2 bg-card border-t border-border ${isIOSMobile() ? "fixed left-0 right-0 bottom-0 pb-[calc(0.5rem+env(safe-area-inset-bottom,0px))] z-[110]" : ""
          }`}
      >
        <div className="flex items-center gap-2 w-full">
          <textarea
            ...
            spellCheck={false}
            autoCapitalize="none"
            className="flex-1 min-w-0 bg-secondary text-foreground border border-border rounded-xl px-3 py-2 text-[16px] leading-5 font-sans outline-none resize-none placeholder:text-foreground/40 focus:ring-1 focus:ring-primary/40 focus:border-primary/40 transition-all"
